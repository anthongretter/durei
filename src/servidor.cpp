...
void Servidor::read(std::string item, int &sockfd, struct sockaddr_in client_addr) {
    ...
    if (it != dataSet.end()) {
        ...
        std::cout << "ITEM " << item << " ENVIADO COM VALOR " << valor << " E VERSÃO " << versao <<
                    " PARA CLIENTE COM IP " << client_addr.sin_addr.s_addr << " E PORTA " <<
        ...
        std::string json_str = resposta.dump(4);
        send(sockfd, json_str.c_str(), json_str.size(), 0);
        std::cout << "ITEM " << item << "PEDIDO PELO CLIENTE COM IP " << client_addr.sin_addr.s_addr <<
                     " E PORTA " << client_addr.sin_port << " NÃO ENCONTRADO\n";
    }
}
            ...
            for (const auto& [item, valor] : json_pedido["ws"].items()) {
                ...
                if (it != dataSet.end()) {
                    ...
                    int versao = vetor[1].get<int>() + 1;
                     std::vector<nlohmann::json>{valor, versao};
                } else {
                    std::vector<nlohmann::json>{valor, 0};
                }
            }
...
void Servidor::lidarComMensagem(int cliente_fd, const std::string& menssagem, struct sockaddr_in client_addr) {
    nlohmann::json json_m = nlohmann::json::parse(menssagem);
    if (json_m["tipo"] == "read") {
        read(json_m["item"], cliente_fd, client_addr);
        ...
        while (true) {
            auto it = std::find_if(
                ...
                [this](const nlohmann::json& j) {
                    return j == this->proxima_mensagem;  // Acessa o membro 'proxima_mensagem' da classe
                }
            );
            if (it != pendentes.end()) {
                tentarComitar(*it);
